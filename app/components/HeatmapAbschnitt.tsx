...
function quintilBreaks(tagesMap: Map<string, number>): [number, number, number] {
  ...
  if (werte.length < 3) return [60, 240, 420]
...
export function HeatmapAbschnitt() {
  ...
  const breaks = quintilBreaks(aggregiereNachTag(
    alleSchichten.filter((s) => s.datum >= iso12MonateVor),
    selectedAgId,
  ))

  // Monatsdaten
  const monatPrefix = `${jahr}-${String(monat).padStart(2, "0")}`
  const tagesMapMonat = aggregiereNachTag(
    alleSchichten.filter((s) => s.datum.startsWith(monatPrefix)),
    selectedAgId,
  )
  ...
  const tooltipZeilen = (() => {
    ...
    const map = new Map<string, number>()
    for (const s of alleSchichten) {
      if (s.datum !== selectedDatum) continue
      if (selectedAgId && s.employerId !== selectedAgId) continue
  ...
  return (
    <section className="mt-8">
      {/* Abschnitts-Header mit Navigation */}
      ...
      {arbeitgeber.length > 1 && (
        <div className="flex gap-2 mb-4 flex-wrap">
          <FilterPill label="Alle" aktiv={selectedAgId === null} farbe="#a8a29e"
            onClick={() => { setSelectedAgId(null); setSelectedDatum(null) }} />
          {arbeitgeber.map((ag) => (
            <FilterPill key={ag.id} label={ag.name} aktiv={selectedAgId === ag.id} farbe={ag.farbe}
              onClick={() => { setSelectedAgId(ag.id); setSelectedDatum(null) }} />
          ))}
        </div>
      )}
      ...
      {/* Tooltip */}
      {selectedDatum && (
        <div className="mt-3 sf-card rounded-2xl p-4 shadow-sm">
            ...
            <>
              {tooltipZeilen.map(({ ag, min }) => (
                <div key={ag?.id ?? "?"} className="flex items-center gap-3 mb-2 last:mb-0">
                  <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: ag?.farbe ?? "#a8a29e" }} />
